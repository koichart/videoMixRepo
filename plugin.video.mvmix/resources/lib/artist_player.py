...
class mvmixArtistPlayer(xbmc.Player):
    ...
    def onPlayBackStarted(self):
        print '[mvmixArtistPlayer] playback started'
        self.sleep(200)
        title = 'Now playing:'
        ...
        if (xbmc.Player().isPlayingVideo()):
            try: result = (int(xbmc.getInfoLabel('VideoPlayer.PlaylistLength')) - int(xbmc.getInfoLabel('VideoPlayer.PlaylistPosition')))
            except: pass
            if result < 2:
                self.add_video()
        if (xbmc.Player().isPlayingVideo()):
    ...
    def onPlayBackStopped(self):
        print '[mvmixPlayer] playback stopped'
    ...
    def add_video(self,artist=False):
        print '[mvmixPlayer] add video'
        video_url = None
        loops = 0
        while not video_url and not xbmc.abortRequested and self.is_active:
            self.set_artist()
            print '[mvmixPlayer] loops: %s' % str(loops)
            print '[mvmixPlayer] artist: %s' % str(self.artist)
            videos = __videos__.get_videos(self.artist)
            videos = self.remove_added_videos(videos)
            print '[mvmixPlayer] videos found: %s' % str(len(videos))
            if videos:
