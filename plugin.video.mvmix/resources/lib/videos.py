...
def get_videos(artist):
    ...
    result = []
    threads = []
    for site in sites: threads.append(threading.Thread(target=videos_thread, args=(site,artist,result)))
    ...
    print '[mvmixPlayer] mode: %s - time: %s' % ('get_videos',str(complete_time))
    return videos
...
def videos_thread(site,artist,result):
    ...
    complete_time = time.time() - start_time
    print '[mvmixPlayer] site: %s - time: %s' % (site,str(complete_time))
    return result
...
def clean(title):
    ...
    title = re.sub(' and | und |der |die |das |the ','', title)
